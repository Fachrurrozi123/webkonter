...
$tabel = mysqli_query($conn, 'SELECT * FROM transaksi_gopay ORDER BY tanggal DESC');
if (!$tabel) {
...
$ambil_tabel_gopay = mysqli_fetch_assoc($tabel_total_seluruh);
//akhir
?>
...
<div class="container py-5">
    <h1 class="text-center fw-bold">Transaksi gopay</h1>
</div>
...
if (isset($_POST['submit'])) {
    ...
    $tambah = mysqli_query($conn, "INSERT INTO transaksi_gopay (tanggal, keterangan_transaksi, nominal) VALUES (NOW(), '$keterangan', '$nominal')");

    // Kurangi total_dana jika keterangan adalah 'transfer'
    if ($keterangan == 'transfer') {
        $total_gopay = $ambil_tabel_gopay['total_gopay'] - $nominal;
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    ...
                    <a href="gopay_hapus.php?id=<?= $tampil['id'] ?>&nominal=<?= $tampil['nominal'] ?>&keterangan_transaksi=<?= $tampil['keterangan_transaksi'] ?>" class="btn btn-danger">Hapus</a>
                </td>
            </tr>
            <?php } ?>
        </tbody>
