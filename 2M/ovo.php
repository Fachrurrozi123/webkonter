...
$tabel = mysqli_query($conn, 'SELECT * FROM transaksi_ovo ORDER BY tanggal DESC');
if (!$tabel) {
...
$ambil_tabel_ovo = mysqli_fetch_assoc($tabel_total_seluruh);
//akhir
?>
...
<div class="container py-5">
    <h1 class="text-center fw-bold">Transaksi Ovo</h1>
</div>
...
if (isset($_POST['submit'])) {
    ...
    $tambah = mysqli_query($conn, "INSERT INTO transaksi_ovo (tanggal, keterangan_transaksi, nominal) VALUES (NOW(), '$keterangan', '$nominal')");

    // Kurangi total_dana jika keterangan adalah 'transfer'
    if ($keterangan == 'transfer') {
        $total_ovo = $ambil_tabel_ovo['total_ovo'] - $nominal;
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    ...
                    <a href="ovo_hapus.php?id=<?= $tampil['id'] ?>&nominal=<?= $tampil['nominal'] ?>&keterangan_transaksi=<?= $tampil['keterangan_transaksi'] ?>" class="btn btn-danger">Hapus</a>
                </td>
            </tr>
            <?php } ?>
        </tbody>
